...
class Analyzer():
    def __init__(self):
        ...
        self.files = [self.client.files.upload(file =tmp_file) for tmp_file in sorted(Path(prompt_dir).glob("*"))]
    ...
    def analyze(self,images,description):
        ...
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text="""- 세미 오버 실루엣
- 가먼트 워싱
- 체크 린넨 원단
- 논카라 디테일
- 아웃포켓 디테일
...
"""),
        ...
        types.Content(
            role="model",
            parts=[
                types.Part.from_text(text="""Here's my thinking process to arrive at the desired JSON output:
...
```"""),
        ...
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text="""MFPEN 은 2015년에 덴마크 코펜하겐을 베이스로 설립된 남성복 브랜드 입니다. 
...
· Machine wash cold"""),
        ...
        types.Content(
            role="model",
            parts=[
                types.Part.from_text(text="""The user wants me to extract specific information about a piece of clothing from provided images and text and present it in a JSON format.
...
```"""),
        ...
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text="""제품명 : BK Cigar co. Heavyweight Tee Off White
...
탄탄한 내구성과 균일한 조직감을 지닌 OE + CD 10's 원사로 제작된 에스피오나지의 헤비웨이트 티셔츠 고중량 원단 특유의 견고함에 덴타 덤블 가공을 거쳐 세탁후에도 변형과 수축을 회소화하고 더욱 안정적인 실루엣을 완성했습니다 기존보다 한층 부드러워진 텍스처로 편안한 착용감을 더한 에스피오나지의 헤비 웨이트 티셔츠를 직접 경험해보세요 """),
        ...
        types.Content(
            role="model",
            parts=[
                types.Part.from_text(text="""The user wants to extract key information about a t-shirt from images and provided text description and format it as a JSON object.
...
```"""),
        ...
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text="""Smoking Rat(Peeing) Heavyweight Tee
...
```"""),
        ...
        types.Content(
            ...
            parts=[
                ...
                types.Part.from_text(text="""제품명 : 멘즈 토렌트쉘 3L 레인 재킷 / 85241P7
    ...
    원단은 블루사인(bluesign®) 인증 원단"""),
            ...
            types.Content(
                role="model",
                parts=[
                    types.Part.from_text(text="""Here's my thinking process to arrive at the desired JSON output based on the user's request:
    ...
    ```"""),
        ...
        generate_content_config = types.GenerateContentConfig(
            ...
            system_instruction=[
                types.Part.from_text(text="""옷의 이미지, 그리고 옷에 대한 러프한 설명을 입력으로 줄거야
...
def generate(
    images: List[Union[io.BytesIO, str]], 
    description: str,
    ):
    ...
    # Create parts list for the model
    parts = []
    for image in images:
        parts.append(convert_pil_image_for_gemini(image))
    ...
    files = [client.files.upload(file =tmp_file) for tmp_file in sorted(Path(prompt_dir).glob("*"))]
...
if __name__ == "__main__": 
    ...
    images =   [Image.open(example_image).convert("RGB") for example_image in Path(example_images).glob("*")]


    analyzer = Analyzer()
    json_response=analyzer.analyze(images = images,
             description= 'blue wool shirt')
    print(json_response)
