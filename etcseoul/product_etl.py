...
class ETC_ProductETL(BaseProductETL):
    ...
    def _transform_single_product(self, product: dict) -> dict:
        ...
        if (m):
            ...
            name = raw  # 혹시 패턴이 안 맞으면 원본 그대로 두기
        product["name"] = name #패턴 매치가 되면 name, 없으면 그냥 raw name
        ...
        image_entries = [ ]
        for index,(image_url, image) in enumerate(zip(
            image_urls, images
        )):
            entry = {
                        "clothing_only": False,
        ...
        product["thumbnail_key"] = image_entries[0][
            "key"
        ] 
